...
public class Listas_Exemplo_7 {
	...
	public static void main(String[] args) {
		...
		ArrayList<Integer> dobrosComStream = IntStream.range(0, 5)
			.map(x -> x * 2)
			.boxed()
			.collect(Collectors.toCollection(ArrayList::new));
		
		System.out.println("Dobos com list comprehesion (Stream): "+ dobrosComStream);
		
		
		System.out.println(
			"Dobros com list comprehesion (enxuto): "+
		IntStream.range(0,5)
				 .map(x -> x *2)
				 .boxed()
				 .toList()
